...
| 4 | landed | board document, frame, and file (`4_board/{0_board,1_boardFrame,2_boardFile}.ts`), and the live surface: `3_boardHost.ts` (gestures as a journal, undo as a cursor), `4_boardRender.ts` + `4_board/react/` (a board you can drag), `site/boardEndpoints.ts` (a dev-only GET/PUT that writes through `writeBoardFile`), and the page at `/grapht/board` | place a section and a fence on a board, move both, reload, positions persist — measured: drag to `translate(240px, 160px)`, Save, reload, still there, read back from `out/board-demo/notes.md.board.json` |
