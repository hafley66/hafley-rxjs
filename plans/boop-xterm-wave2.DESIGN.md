...
| 9 | row 8 point lookup (lane stop, `00a_terminalIntersection.ts:79`) | `ViewportSnapshot` gains `geometry: { top: number; cellHeight: number; viewportY: number }`, sampled from the terminal element and buffer when the snapshot is taken. Pure exports: `bufferRowAtPoint(geometry, point): number \| null` (body copied from the existing conversion) and `turnAtBufferRow(visible, row)` / `regionAtBufferRow(visible, row)`. Callers compose them: `turnAtBufferRow(visibility.state.visible.$(), bufferRowAtPoint(viewport.snapshot.geometry.$(), point))`. |
