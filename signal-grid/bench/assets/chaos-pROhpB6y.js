import { i as mountBench, n as DEFAULTS, o as setGridLogEmit, s as cellId, t as COLUMNS } from "./0_bench-CoT3Z-06.js";
//#region bench/scroll/chaos.ts
var must = (id) => {
...
var paint = () => {
	...
	readout.innerHTML = `<b>${fps.toFixed(0)} fps</b> ${mean.toFixed(1)} ms mean, p95 ${p95.toFixed(1)} ms &middot; <b class="${slow === 0 ? "ok" : "bad"}">${slow}</b> frames over 32 ms of ${at} &middot; <b>package ${pkgPerFrame.toFixed(2)} ms/f</b>, ${share.toFixed(0)}% of the frame (${stages}) &middot; ${rows} rows, ${nodes} nodes &middot; heap ${mb(heap?.usedJSHeapSize)}`;
};
var tick = (now) => {
