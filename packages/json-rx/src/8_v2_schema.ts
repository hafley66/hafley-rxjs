import * as z from "zod";

const JsonPrimitiveSchema = z.union([z.null(), z.boolean(), z.number(), z.string()]);

const ParameterSchema = z.strictObject({
  type: z.enum(["boolean", "integer", "number", "string"]),
  default: JsonPrimitiveSchema.optional(),
});

const ParametersSchema = z.strictObject({
  path: z.record(z.string(), ParameterSchema).optional(),
  query: z.record(z.string(), ParameterSchema).optional(),
});

const SourceExpressionSchema = z.strictObject({
  node: z.string().min(1),
  source: z.strictObject({ ref: z.string().min(1) }),
});

const HostExpressionSchema = z.strictObject({
  node: z.string().min(1),
  host: z.strictObject({
    ref: z.string().min(1),
    input: z.lazy(() => ObservableExpressionSchema),
  }),
});

const MapExpressionSchema = z.strictObject({
  node: z.string().min(1),
  map: z.strictObject({
    input: z.lazy(() => ObservableExpressionSchema),
    from: z.string().regex(/^\$\.(?:[A-Za-z_][A-Za-z0-9_]*\.?)+$/),
    language: z.literal("jsonata").default("jsonata"),
    fields: z.record(z.string().min(1), z.string().min(1)),
  }),
});

const ShareReplayExpressionSchema = z.strictObject({
  node: z.string().min(1),
  shareReplay: z.strictObject({
    input: z.lazy(() => ObservableExpressionSchema),
    bufferSize: z.literal(1),
    refCount: z.literal(true),
  }),
});

const MergeExpressionSchema = z.strictObject({
  node: z.string().min(1),
  merge: z.strictObject({ inputs: z.array(z.lazy(() => ObservableExpressionSchema)).min(1) }),
});

const ScanExpressionSchema = z.strictObject({
  node: z.string().min(1),
  scan: z.strictObject({
    input: z.lazy(() => ObservableExpressionSchema),
    reducer: z.strictObject({ ref: z.string().min(1) }),
  }),
});

const ObservableExpressionSchema: z.ZodType<ObservableExpression> = z.lazy(() => z.union([
  SourceExpressionSchema,
  HostExpressionSchema,
  MapExpressionSchema,
  MergeExpressionSchema,
  ScanExpressionSchema,
  ShareReplayExpressionSchema,
]));

export type ObservableExpression =
  | z.infer<typeof SourceExpressionSchema>
  | { node: string; host: { ref: string; input: ObservableExpression } }
  | {
      node: string;
      map: {
        input: ObservableExpression;
        from: string;
        language: "jsonata";
        fields: Record<string, string>;
      };
    }
  | { node: string; merge: { inputs: ObservableExpression[] } }
  | { node: string; scan: { input: ObservableExpression; reducer: { ref: string } } }
  | {
      node: string;
      shareReplay: { input: ObservableExpression; bufferSize: 1; refCount: true };
    };

const NetworkResponseBindingSchema = z.strictObject({
  kind: z.literal("browser.network.response"),
  page: z.strictObject({ host: z.string().min(1) }),
  request: z.strictObject({
    methods: z.array(z.string().min(1)).min(1),
    url: z.string().min(1),
  }),
});

const HostEventBindingSchema = z.strictObject({
  kind: z.literal("host.event"),
  operation: z.string().min(1),
});

const HostPortSchema = z.strictObject({
  kind: z.enum(["source", "operator"]),
  inputSchema: z.string().min(1).optional(),
  outputSchema: z.string().min(1),
  config: z.record(z.string(), z.unknown()).optional(),
  delivery: z.strictObject({
    subscription: z.enum(["per-subscriber", "shared"]),
    replay: z.enum(["none", "last"]),
  }),
  capabilities: z.array(z.string().min(1)).default([]),
}).superRefine((port, context) => {
  if (port.kind === "operator" && port.inputSchema === undefined) {
    context.addIssue({ code: "custom", path: ["inputSchema"], message: "operator port requires inputSchema" });
  }
  if (port.kind === "source" && port.inputSchema !== undefined) {
    context.addIssue({ code: "custom", path: ["inputSchema"], message: "source port cannot declare inputSchema" });
  }
});

const CircuitSourceSchema = z.strictObject({
  parameters: ParametersSchema.optional(),
  host: z.strictObject({ ref: z.string().min(1) }).optional(),
});

const ReducerCaseSchema = z.strictObject({
  replace: z.string().min(1).optional(),
  patch: z.record(z.string().min(1), z.string().min(1)).optional(),
}).refine(
  (reducerCase) => (reducerCase.replace !== undefined) !== (reducerCase.patch !== undefined),
  { message: "reducer case must specify exactly one of replace or patch" },
);

const ReducerSchema = z.strictObject({
  seed: z.record(z.string(), z.unknown()),
  cases: z.record(z.string().min(1), ReducerCaseSchema),
});

const DashboardOutputSchema = z.strictObject({
  kind: z.literal("instant.dashboard.emit"),
  flow: z.string().min(1),
  stream: z.string().min(1),
  schema: z.record(z.string(), z.unknown()),
});

export const AutomationV2Schema = z.strictObject({
  version: z.literal("automation.v2"),
  profile: z.literal("rxjs-7.8"),
  id: z.string().min(1),
  enabled: z.boolean().default(true),
  bindings: z.strictObject({
    sources: z.record(z.string().min(1), z.union([NetworkResponseBindingSchema, HostEventBindingSchema])),
    hosts: z.record(z.string().min(1), HostPortSchema).default({}),
  }),
  circuit: z.strictObject({
    sources: z.record(z.string().min(1), CircuitSourceSchema),
    flows: z.record(z.string().min(1), z.strictObject({
      parameters: ParametersSchema.optional(),
      expression: ObservableExpressionSchema,
    })),
    reducers: z.record(z.string().min(1), ReducerSchema).default({}),
  }),
  outputs: z.array(DashboardOutputSchema).min(1),
}).superRefine((automation, context) => {
  const sourceRefs = new Set(Object.keys(automation.circuit.sources));
  const flowRefs = new Set(Object.keys(automation.circuit.flows));
  const nodes = new Set<string>();

  for (const bindingRef of Object.keys(automation.bindings.sources)) {
    if (!sourceRefs.has(bindingRef)) {
      context.addIssue({
        code: "custom",
        path: ["bindings", "sources", bindingRef],
        message: `binding references unknown source: ${bindingRef}`,
      });
    }
  }
  for (const sourceRef of sourceRefs) {
    const source = automation.circuit.sources[sourceRef];
    if (source.host) {
      const port = automation.bindings.hosts[source.host.ref];
      if (!port) {
        context.addIssue({ code: "custom", path: ["circuit", "sources", sourceRef, "host", "ref"], message: `unknown host port: ${source.host.ref}` });
      } else if (port.kind !== "source") {
        context.addIssue({ code: "custom", path: ["circuit", "sources", sourceRef, "host", "ref"], message: `host port is not a source: ${source.host.ref}` });
      }
      continue;
    }
    if (!Object.prototype.hasOwnProperty.call(automation.bindings.sources, sourceRef)) {
      context.addIssue({
        code: "custom",
        path: ["bindings", "sources"],
        message: `source has no host binding: ${sourceRef}`,
      });
    }
  }

  const visit = (expression: ObservableExpression, flowRef: string) => {
    if (nodes.has(expression.node)) {
      context.addIssue({
        code: "custom",
        path: ["circuit", "flows", flowRef, "expression"],
        message: `duplicate node id: ${expression.node}`,
      });
    }
    nodes.add(expression.node);
    if ("source" in expression && !sourceRefs.has(expression.source.ref)) {
      context.addIssue({
        code: "custom",
        path: ["circuit", "flows", flowRef, "expression", "source", "ref"],
        message: `unknown source: ${expression.source.ref}`,
      });
    }
    if ("host" in expression) {
      const port = automation.bindings.hosts[expression.host.ref];
      if (!port) {
        context.addIssue({ code: "custom", path: ["circuit", "flows", flowRef, "expression", "host", "ref"], message: `unknown host port: ${expression.host.ref}` });
      } else if (port.kind !== "operator") {
        context.addIssue({ code: "custom", path: ["circuit", "flows", flowRef, "expression", "host", "ref"], message: `host port is not an operator: ${expression.host.ref}` });
      }
      visit(expression.host.input, flowRef);
    }
    if ("map" in expression) visit(expression.map.input, flowRef);
    if ("merge" in expression) expression.merge.inputs.forEach((input) => visit(input, flowRef));
    if ("scan" in expression) {
      if (!automation.circuit.reducers[expression.scan.reducer.ref]) {
        context.addIssue({
          code: "custom",
          path: ["circuit", "flows", flowRef, "expression", "scan", "reducer", "ref"],
          message: `unknown reducer: ${expression.scan.reducer.ref}`,
        });
      }
      visit(expression.scan.input, flowRef);
    }
    if ("shareReplay" in expression) visit(expression.shareReplay.input, flowRef);
  };

  for (const [flowRef, definition] of Object.entries(automation.circuit.flows)) {
    visit(definition.expression, flowRef);
  }
  for (const [index, output] of automation.outputs.entries()) {
    if (!flowRefs.has(output.flow)) {
      context.addIssue({
        code: "custom",
        path: ["outputs", index, "flow"],
        message: `output references unknown flow: ${output.flow}`,
      });
    }
  }
});

export type AutomationV2 = z.infer<typeof AutomationV2Schema>;

export const AutomationV2JsonSchema = z.toJSONSchema(AutomationV2Schema, {
  target: "draft-2020-12",
  cycles: "ref",
  reused: "ref",
});
