...
export function AnimationControls({
  label, time, duration = 1, running, loop = true, onSeek, onToggle, onReplay,
}: {
  label: string
  time: number
  duration?: number
  ...
  return <div className="my-3 flex flex-wrap items-center gap-3" data-animation-controls={label}>
    ...
    <button className={BUTTON} title="Save this frame in the URL and selected named state" onClick={() => onSeek(time)}>Hold</button>
