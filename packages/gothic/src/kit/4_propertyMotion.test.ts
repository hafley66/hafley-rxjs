...
it("shares a cold stroke clock and applies global or cascading settings only with field permission", () => {
  ...
  const config = Signal<PropertyMotionConfig>({ transport: { time: 0, speed: 1, run: true }, timing: DEFAULT_TIMING,
    variation: { scope: "stroke", seed: 17 }, sections: { art: {
      timing: {}, variation: { seed: 23 }, fields: { weight: { enabled: true, variationPolicy: "none", variation: { seed: 31 }, timing: {}, frames: [] } },
    } },
  })
  let listeners = 0
  const model = createPropertyMotion(config, ticks.pipe(tap({ subscribe: () => listeners++, finalize: () => listeners-- })))
  const state = { page: "slice", id: "art", spec, values: base } as unknown as SectionState<AnySpec>
  ...
  const record = () => receipt.push({ listeners, time: strokes.time.$(), fieldTime: strokes.times.weight.$(), period: strokes.fields.weight.period.$(), seed: strokes.fields.weight.seed.$(), base: base.$(), values: values.$() })
  record()
  ...
  config.sections.art.timing.$({ delay: 100, duration: 2000 })
  config.sections.art.fields.weight.timing.$({ delay: 600, duration: 1000 }); record()
  ticks.next(250); record()
  config.sections.art.fields.weight.timing.$({}); record()
