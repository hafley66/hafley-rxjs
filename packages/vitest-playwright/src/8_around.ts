...
import { aroundEach, inject, type TestContext, vi } from "vitest"
import { KEY, type ResolvedOptions } from "./0_options.js"
import type { PwFile, PwTest, PwWorker } from "./4_test.js"
import { acquire, attemptEvents$, browser$, context$, type Handle, page$, real } from "./5_streams.js"
import {
  active,
  als,
  attemptSignal,
...
export async function ensureBrowser(o: ResolvedOptions) {
  const existing = workerState.root?.browser.$()
  if (existing) return existing
  lazyBrowser ??= acquire(browser$(o)).catch(e => {
...
function realTimers(task: Test, phase: string) {
  ...
  vi.useRealTimers()
  logBridge("vi.useFakeTimers was still installed at {phase} of {name}; real timers restored", {
    phase,
    name: task.name,
    testId: task.id,
  })
...
// vitest: hooks resolve the fixtures they destructure, so the hook context is the test context plus whatever pkg:test defines
type HookContext = TestContext & Partial<PwWorker & PwFile & Pick<PwTest, "contextOptions">>
  ...
  aroundEach(
    async (
      ...
      const contextOptions: BrowserContextOptions = {
        ...o.context,
        baseURL: baseURL ?? o.context.baseURL,
        ...(ctxOverride ?? {}),
      }
      ...
      const root = testRoot(
        task,
        attemptSignal(signal, task.timeout, (ms, cb) => real.setTimeout(cb, ms)),
        o,
      )
      const log = testLog(
        attemptEvents$(root).pipe(
          tap(ev => {
            if (o.log.page) logEvent(task.id, ev)
          }),
        ),
      )
        ...
        try {
          if (fileRoot) {
            ...
            if (owner && owner !== task.id)
              throw new Error(
                `contextScope 'file': test ${task.name} started while ${owner} owns the page; file scope requires sequential tests`,
              )
          ...
          try {
            ...
            if (page && context) await capture(o, task, page, context, dir, failed, fileRoot ? "chunk" : "stop")
            root.phase.$("release")
...
async function capture(
    ...
    budget.aborted
      ...
      : Promise.race([
          ...
          new Promise<never>((_, rej) =>
            budget.addEventListener("abort", () => rej(new Error("artifact budget exceeded")), { once: true }),
          ),
        ])
  const wantShot = o.artifacts.screenshot === "on" || (o.artifacts.screenshot === "only-on-failure" && failed)
  const wantTrace =
    o.artifacts.trace === "on" ||
    (o.artifacts.trace === "retain-on-failure" && failed) ||
    (o.artifacts.trace === "on-first-retry" && (task.result?.retryCount ?? 0) === 1)
    ...
    try {
      ...
      if (failed)
        await recordArtifact(task, {
          ...
          attachments: [{ path, originalPath: path, contentType: "image/png" }],
        })
    } catch (e) {
      logBridge("screenshot skipped for {name}: {error}", {
        name: task.name,
  ...
  if (o.artifacts.trace !== "off") {
    ...
    try {
      await bounded(tracing === "chunk" ? context.tracing.stopChunk({ path }) : context.tracing.stop({ path }))
    } catch (e) {
      logBridge("trace stop skipped for {name}: {error}", {
        name: task.name,
        error: String((e as Error)?.message ?? e),
      })
    }
...
function assertLog(o: ResolvedOptions, log: TestLog, task: Test) {
  const l = log.$()
  const errors = [
    ...(o.log.failOnPageError ? l.errors.map(e => `pageerror ${e.message} (${e.url})`) : []),
    ...(o.log.failOnConsoleError
      ? l.console.filter(c => c.type === "error").map(c => `console.error ${c.text} (${c.url})`)
      : []),
  ]
  if (errors.length && task.result?.state !== "fail")
    throw new Error(`page errors during "${task.name}":\n  - ${errors.join("\n  - ")}`)
}
