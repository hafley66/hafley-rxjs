...
| `workers` | `workerBudget().workers` | vitest `test.maxWorkers` unless the user config sets it. The default is read at config time: `min(cores / 2, availableMemory / 1.2 GB, idleCores / 2)`, floored at 1, where available memory is `vm_stat` free + inactive + speculative + purgeable on darwin and `MemAvailable` on linux (`src/12_budget.ts`). One browser per worker, so vitest's `cores - 1` default means that many chromiums |
