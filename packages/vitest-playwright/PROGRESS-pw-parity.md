...
| pw-teardown | teardown order: test scope, afterAll, worker scope, reverse creation | `PW/worker/workerProcessEntry.js:268-284,1692-1704` | unverified | `src/9_runner.ts` pwp:pw-teardown | child run writes an order log: `context.close` before `afterAll` before `browser.close` |
