...
// three() Renderer over the @hafley66/scene pipeline: meshes by id with a pool, cards as CSS2DObject,
...
const xOf = (page: import("@playwright/test").Page, id: string) =>
  page.evaluate((target) => {
    const host = document.getElementById("host") as HTMLElement & { __threeScene?: { views: Map<string, { position: { x: number } }> } }
    return host.__threeScene?.views.get(target)?.position.x ?? null
  }, id)

test("mounts, draws scene 0 with 300 meshes and 4 CSS2D cards", async ({ page }) => {
  const errors: string[] = []
...
const viewOf = (page: import("@playwright/test").Page, id: string, tag: string) =>
  page.evaluate(
    ([target, mark]) => {
      const host = document.getElementById("host") as HTMLElement & { __threeScene?: { views: Map<string, { __tag?: string }> } }
      const view = host.__threeScene?.views.get(target)
      if (!view) return null
...
test("step 0 -> 1 keeps n100..n299, recycles the 100 exited meshes into the 100 entrants, and tweens a kept id", async ({ page }) => {
  await page.goto("/labs/scene-grid.html")
  await page.waitForFunction(() => (window as unknown as { __lab: Lab }).__lab.inspect()?.ready === true)
  ...
  const midHandle = await page.waitForFunction(
    (start) => {
      const host = document.getElementById("host") as HTMLElement & { __threeScene?: { views: Map<string, { position: { x: number } }> } }
      const x = host.__threeScene?.views.get("n150")?.position.x
...
test("step 2 ring reuses pooled meshes, then unsubscribe removes the canvas", async ({ page }) => {
  await page.goto("/labs/scene-grid.html")
  await page.waitForFunction(() => (window as unknown as { __lab: Lab }).__lab.inspect()?.ready === true)
  await setStep(page, 1)
