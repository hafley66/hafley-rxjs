...
export type ThreeScenarioState = {
  projection: ThreeProjection | null
  geometry: Geometry
...
export type ThreeScenarioOptions = {
  loadFixture?: (fixture: string) => Promise<ThreeFixture>
}
...
function visible(state: ThreeScenarioState): { nodes: number; edges: number } {
...
function unsupported(state: ThreeScenarioState, scenario: BenchScenario, reason: string): { state: ThreeScenarioState; sample: ScenarioSample } {
  const counts = visible(state)
  return {
    state,
    sample: {
...
): Promise<{ state: ThreeScenarioState; sample: ScenarioSample }> {
  const started = now()
  await operation()
...
function unsupportedHandler<S extends BenchScenario>(scenario: S, reason: string): BenchScenarioHandler<ThreeScenarioState, S, ScenarioSample> {
  return state => unsupported(state, scenario, reason)
}

function requireProjection(state: ThreeScenarioState): ThreeProjection | null {
...
export function createThreeScenarioHandlers(options: ThreeScenarioOptions = {}): BenchScenarioHandlers<ThreeScenarioState, ScenarioSample> {
  return {
    ...
    "device-pixel-ratio-change": async (state, args) => {
      const projection = requireProjection(state)
      if (!projection) return unsupported(state, "device-pixel-ratio-change", "graph has been disposed")
      return supported(state, "device-pixel-ratio-change", () => { projection.setDevicePixelRatio(args.pixelRatio); projection.render() }, 2)
    },
    "group-collapse": unsupportedHandler("group-collapse", "ThreeProjection has no group compound-node model"),
    "group-expand": unsupportedHandler("group-expand", "ThreeProjection has no group compound-node model"),
    ...
    "layout-run": unsupportedHandler("layout-run", "Three.js ships no graph layout engine and this adapter embeds none"),
    ...
    "graph-replace": async (state, args) => {
      ...
      const loaded: ThreeFixture = options.loadFixture ? await options.loadFixture(args.fixture) : { geometry: state.geometry, bytes: state.fixtureBytes ?? undefined }
    ...
    "labels-none": unsupportedHandler("labels-none", "ThreeProjection draws no text labels, so label policy has nothing to select"),
    "labels-visible": unsupportedHandler("labels-visible", "ThreeProjection draws no text labels, so label policy has nothing to select"),
    "labels-fixed-count": unsupportedHandler("labels-fixed-count", "ThreeProjection draws no text labels, so label policy has nothing to select"),
    "labels-dense": unsupportedHandler("labels-dense", "ThreeProjection draws no text labels, so label policy has nothing to select"),
  }
}

export function initialThreeScenarioState(
  geometry: Geometry,
  container: HTMLElement | null,
  fixtureBytes: number | null = null,
  projection: ThreeProjection | null = null,
): ThreeScenarioState {
