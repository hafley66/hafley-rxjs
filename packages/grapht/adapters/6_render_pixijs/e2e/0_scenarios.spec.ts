...
async function capture(page: import("@playwright/test").Page, file: string): Promise<PixiReceipt> {
  const text = await page.locator("#receipt").textContent()
  return JSON.parse(text ?? "{}") as PixiReceipt
}

for (const representation of ["retained", "particles"]) {
  test(`renders grid-1k through PixiJS WebGL (${representation}) and records a PNG + JSON receipt`, async ({ page }) => {
    const errors: string[] = []
    page.on("pageerror", error => errors.push(error.stack ?? error.message))
    await page.goto(`/?nodes=1000&renderer=webgl&representation=${representation}&pause=1`)
    await expect(page.locator("#pixi-container[data-visual-valid='true']"), errors.join("\n")).toHaveCount(1, { timeout: 40_000 })
    await expect(page.locator("#pixi-container[data-actual-backend='webgl']"), errors.join("\n")).toHaveCount(1)
