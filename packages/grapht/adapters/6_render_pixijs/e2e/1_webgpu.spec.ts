...
for (const representation of ["retained", "particles"]) {
  test(`renders grid-1k through PixiJS WebGPU or reports it unavailable with WebGL evidence (${representation})`, async ({ page }) => {
    const errors: string[] = []
    page.on("pageerror", error => errors.push(error.stack ?? error.message))
    await page.goto(`/?nodes=1000&renderer=webgpu&representation=${representation}&pause=1`)

    await expect(page.locator("#pixi-container[data-receipt-status='ready'], #pixi-container[data-receipt-status='webgpu-unavailable']"), errors.join("\n")).toHaveCount(1, { timeout: 40_000 })
    ...
    const value = JSON.parse(await receipt.textContent() ?? "{}") as PixiReceipt
