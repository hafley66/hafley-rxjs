...
describe("sequence package gate", () => {
  test("keeps language adapters and the pure model in separate package boundaries", async () => {
    ...
    expect({
      ...
      modelSources,
      nonNumericSources: {
        model: modelSources.filter(filename => !/^\d+[a-z]?_/.test(filename)),
        mmd: mmdSources.filter(filename => !/^\d+[a-z]?_/.test(filename)),
        d2: d2Sources.filter(filename => !/^\d+[a-z]?_/.test(filename)),
      },
      adapterImports: {
  ...
  test("keeps the model pure and geometry parser-free", async () => {
    const [modelSources, geometry] = await Promise.all([
      numberedSources("grapht-model"),
      source("grapht", "1_sequence/3_geometry.ts"),
    ])
