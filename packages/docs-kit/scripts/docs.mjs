// @comment-ok: every check here exists because a document drifted from the tree once, and the header states which source answers each one
// Fails when a document says something the repository does not, answering from the file system, the
// TypeScript program, the test files, and whatever data the caller hands over.
//
// `--fix` repoints a stale `path:line` whose symbol it can find elsewhere in the same file.
// `--render` expands `{{...}}` placeholders into the caller's dist directory.
...
/** The nouns a bare number beside them is a drifting count of, in any package. */
export const CORE_UNITS = new Map([
...
// Options: pkg, workspace, tsconfig, dist, files, roots, generated, indexed, testDirectories,
// units, data, argv. Every path is absolute; every list is relative to `pkg`.
export function runDocs(options) {
  ...
  // Named files replace the sweep, which is how a check is demonstrated on one file without
  // touching the tree the sweep reads.
  const named = argv.filter((it) => !it.startsWith("--")).map((it) => resolve(process.cwd(), it))

  const GENERATED = new Set(options.generated ?? [])
  const INDEXED = options.indexed ?? ["src", "site", "demo", "bench", "docs", "scripts", "tests", "fixtures", "examples", "."]
  const TEST_DIRECTORIES = options.testDirectories ?? ["src", "tests", "bench", "demo", "site"]
  const UNITS = options.units ?? CORE_UNITS
  ...
  const documents = () => {
    ...
    const found = [
      ...
      ...(options.roots ?? []).flatMap((it) => listing(it.dir, it.suffix)),
    ]
  ...
  // A citation writes `src/8_grid.ts` from the package and `packages/signals/src/2_Signal.ts` from
  // the workspace, and a bare `stats.json` from neither, so all three shapes get an answer.
  ...
  // `@hafley66/signal-grid/theme.css` is a package specifier and `.d.ts` is an extension being
  // named, so neither is a claim that a file exists here.
  ...
  const locate = (cited) => {
    ...
    // A `.js` specifier is how TypeScript spells an import of the `.ts` beside it.
  ...
  // A bare file name in a block that links out names a file in that repository, which this one has
  // no answer for. A path carrying a directory is still a claim about this tree and stays checked.
  ...
  // An object answer is not printable, so a group that carries its own headline number under
  // `tests`, `count`, `total`, or `value` collapses to it.
  ...
  // A markdown table row is many sentences on one physical line, so `|` ends a clause as firmly as
  // a full stop does. Without that, a citation borrows the symbol named in the cell beside it.
  ...
  // A name on forty lines cannot be repointed to one of them, and `it` on a line of a test file is
  // not a claim about that line at all. Both cases are silence rather than a guess.
  const AMBIGUOUS = 6
  ...
  const checkCitations = (file, lineNumber, line, spans, external) => {
    for (const span of spans) {
      ...
      report("stale-citation", file, lineNumber, `cites ${span.path}:${span.line} but "${name}" is at :${actual}`)
  ...
  // Scoped to the clause rather than the physical line: a competitor table names another library's
  // API in one cell and this package's file in the next, which is not a claim about each other.
    ...
    for (let index = 0; index < tokens.length; index++) {
      ...
      for (const direction of [1, -1]) {
        for (let step = 1; step <= REACH; step++) {
          ...
          // A table cell wall ends the reach: a number in one column never counts the next noun.
  ...
  // A test name is a sentence a person wrote, so a fragment of TypeScript that happens to sit
  // between quotes is not one. In a `.ts` file only a comment can be quoting a test.
  ...
  // Line by line rather than over the whole text, because a fence showing the placeholder syntax
  // has to survive the pass that expands every other one.
  ...
  if (fixing) {
    ...
    for (const repair of repairs.filter((it) => it.replacement === undefined)) {
  ...
  return { violations, repairs, testFiles: TEST_FILES, testNames: [...testNames] }
}
