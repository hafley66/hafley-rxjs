...
// The demo is served under `/hafley-rxjs/signal-grid/demo/` on pages and `/` in dev, so the base is
// an input. `import.meta.env.BASE_URL` is what vite was built with.
const route = Route("/:demo", { base: import.meta.env.BASE_URL })
...
document.addEventListener("click", (event) => {
  ...
  const hit = route.matchPath(anchor.pathname)
  if (!hit.matched) return
  const slug = String(hit.demo)
