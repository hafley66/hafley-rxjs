...
which takes `buildComparator` alone (`src/8_grid.ts:381`, `:342`). Roughly 44 of the 56 exercise
unwired code.
...
two counts. `src/10_render.ts:66` routes the four glyph built-ins through `ROW_ROUTED` and
