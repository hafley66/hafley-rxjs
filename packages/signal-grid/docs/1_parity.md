...
| `row.expand`<br>Open and close a parent row's children | Every descendant renders at once, so a deep tree floods the viewport on first paint. | yes | [yes (pro)](https://mui.com/x/react-data-grid/tree-data/#group-expansion-with-tree-data) | yes | `src/1_axis.ts:376`<br>`src/5_columns.ts:311` |
...
| `col.formula`<br>Derive a column's value from other fields of the row | A computed column has to be materialised into the source data before it can be shown. | no | [yes (premium)](https://mui.com/x/react-data-grid/formulas/) | declared only | `src/0_types.ts:264` |
