...
`src/8_grid.ts:612` builds as `colPlan`. It is off by default, because a schema narrow enough to fit
`src/8_grid.ts:606` builds as `colPlan`. It is off by default, because a schema narrow enough to fit
