...
Measured 2026-09-13, gzipped: 53.31 kB published javascript, 22.25 kB eager half, 18.26 kB for
`{ grid, render }`, 2.03 kB theme.css (after the ARIA pass and the xdom chord operators). `.size-limit.json` holds the next 0.5 kB above each, so
