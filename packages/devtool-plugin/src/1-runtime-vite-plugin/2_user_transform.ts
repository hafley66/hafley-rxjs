		...
		if (
			...
			const operators = (node.arguments || []).map((op: any) => {
				if (op.type === "CallExpression" && op.callee?.type === "Identifier") {
					...
					const opArgs = (op.arguments || []).map((a: any) =>
						serializeAstNode(a),
					);
...
function isRxjsCreatorCall(
	node: any,
	importedCreators: Set<string>,
	namespaces: Set<string>,
): boolean {
...
function isSubjectConstruction(
	node: any,
...
function isPipeCall(node: any): boolean {
	...
	const callee = node.callee;
	return (
		callee?.type === "MemberExpression" && callee.property?.name === "pipe"
	);
}
...
// File detection
export function shouldTransformUserCode(
	id: string,
...
export function transformUserCode(
	...
	const { creators, subjects, namespaces, lastImportEnd } =
		collectRxjsImports(ast);
