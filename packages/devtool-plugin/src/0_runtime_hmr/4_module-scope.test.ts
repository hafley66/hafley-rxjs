...
describe("ModuleScope", () => {
  ...
  describe("_rxjs_debugger_module_start", () => {
    var lol = []
    zip(from(main.event$).pipe(startWith(null)), main.state$$).subscribe(it => lol.push(JSON.stringify(it, null, 4)))
    ...
    it("increments version on HMR reload", () => {
      ...
      expect(lol).toMatchInlineSnapshot()
      expect(main.state$.value.store.hmr_module[url]?.version).toBe(1)
    ...
    it("stamps module_id on hmr_track entities", () => {
      ...
      __$("myObs", () => {
        expect(main.state$.value.stack.hmr_track).toMatchInlineSnapshot(`
          [
            {
              "created_at": 0,
              "id": "0",
        ...
        `)
        expect(main.state$.value.stack.hmr_module).toMatchInlineSnapshot(`
          [
            {
              "created_at": 0,
              "id": "file:///c.ts",
      ...
      expect(findTrackByKey(main.state$.value, "myObs")?.module_id).toBe("file:///c.ts")
    })
  })
  ...
  describe("nested keys", () => {
    it("concatenates keys with colon delimiter", () => {
      ...
      expect(findTrackByKey(main.state$.value, "outer:inner")).toBeDefined()
      expect(findTrackByKey(main.state$.value, "outer:inner")?.module_id).toBe("file:///d.ts")
    })
    ...
    it("deeply nested keys work correctly", () => {
      ...
      expect(findTrackByKey(main.state$.value, "a:b:c")).toBeDefined()
    })
  })
  ...
  describe("orphan detection", () => {
    it("snapshots prev_keys on HMR reload", () => {
      ...
      expect(main.state$.value.store.hmr_module["file:///f.ts"]?.prev_keys).toEqual([])
    })
    ...
    it("cleans up orphaned tracks: deletes from store and completes wrapper", () => {
      ...
      const trackKeysBefore = Object.values(main.state$.value.store.hmr_track)
        .map(t => t.key)
      ...
      const trackKeysAfter = Object.values(main.state$.value.store.hmr_track)
        .map(t => t.key)
        .filter(k => k.startsWith("orphan_"))
  ...
  describe("defer/lazy factory", () => {
    it("defer factory runs with subscription context", () => {
      ...
      const relevantTracks = Object.fromEntries(
        Object.values(main.state$.value.store.hmr_track)
