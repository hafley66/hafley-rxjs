    ...
    this.state$.scanEager((state, event) => {
      ...
      switch (event.type) {
        ...
        case "factory-call-return": {
          ...
          console.log("and id:", obsId, event)
        ...
        case "send-call": {
          ...
          if (originTrack) state.stack.hmr_track.push(originTrack)
        ...
        case "fun-call": {
          state.store.call[event.id] = {
            created_at: this.now(),
            id: event.id,
          }
          event.args.forEach(it => {
            state.store.arg2[it.id] = it
            if (state.store.observable[it.obs_id ?? ""]) {
              state.store.observable[it.obs_id ?? ""]!.name = it.id
            }
          })
          break
        }
        case "fun-call-return": {
          state.store.call[event.id]!.created_at_end = this.now()
          state.store.arg2[event.$return.id] = event.$return
          if (state.store.observable[event.$return.obs_id ?? ""]) {
            state.store.observable[event.$return.obs_id ?? ""]!.name = event.$return.id
          }
          break
        }
      }
  ...
  _1_findArgs(value: unknown, path = "$args", depth = 0, rootVal: any): ArgEntity[] {
    ...
    if (isFunction(value)) {
      ...
      set(rootVal, path, (...callArgs: any[]) => {
        ...
        const fn = this.state$.value.store.arg[arg_id]?.fn_ref
...
export function patchObservable(Observable: { prototype: any; create?: any }) {
  ...
  /* 
  pipe(   // fun(pipe).call()
    a(),  //   fun.call.return.fun -> call.arg.1
    b(),  //   fun.call.return.fun -> call.arg.2
  ) -> 
    fun(pipe).call.arg.1.fun(a).call.arg.1 -> observable
    fun(pipe).call.arg.1.fun(a).call.return -> observable
    fun(pipe).call.arg.2.fun(b).call.arg.1 -> observable
    fun(pipe).call.arg.2.fun(b).call.return -> observable
