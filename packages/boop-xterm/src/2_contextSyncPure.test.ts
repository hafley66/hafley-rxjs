...
import { turnsAcrossRange, type PromptContextItem } from "./1_contextQueuePure.js";
import { diffItems, rowShowsOn, shapeOf, splitTurnId, toComment, toItem, type BoopTurnComment } from "./2_contextSyncPure.js";
...
describe("turnsAcrossRange", () => {  const turn = (id: string, role: string, anchorStart: number, anchorEnd: number) =>
    ({ id, role, anchorStart, anchorEnd, session: id.slice(0, id.lastIndexOf(":")), harness: "omp", turn: Number(id.slice(id.lastIndexOf(":") + 1)), ts: 0, said: "", bufferStart: anchorStart, bufferEnd: anchorEnd, regions: [], confidence: "anchored", source: "xterm+boop" }) satisfies VisibleTurn;
