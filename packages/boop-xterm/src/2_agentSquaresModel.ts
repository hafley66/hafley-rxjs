...
import { SQUARE_STEP, type SquareKind } from "./0_agentSquareVisual.js"
import type { Strip, StripTurn } from "./1_agentSquaresFeed.js"
...
/** Apply the resolved square step to band and recent placement. */
export function themedSquaresOf(frame: Strip, geometry: SquareGeometry, recent: number, step: number): AgentSquaresProps {
  const projected = squaresOf(frame, geometry, recent)
  if (!frame.layout || step === SQUARE_STEP) return projected
  if (frame.layout.mode === "relative") {
    projected.squares.forEach((square, index) => { if (square.pinned) square.y = index * step })
  } else {
    const block = Math.max(0, projected.squares.length - 1) * step
    const start = Math.max(0, (geometry.track - block) / 2) + recentOffset(recent, geometry.track, block)
    projected.squares.forEach((square, index) => { square.y = start + index * step })
  }
  const before = projected.squares.find((square) => square.id === frame.layout?.gap?.beforeId)
  const after = projected.squares.find((square) => square.id === frame.layout?.gap?.afterId)
  if (projected.gap) projected.gap.y = before && after ? (before.y + after.y) / 2
    : before ? before.y + step / 2 : after ? after.y - step / 2 : geometry.track / 2
  return projected
}
