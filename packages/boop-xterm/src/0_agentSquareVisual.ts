...
export type SquareVisual = SignalType<SquareState>
...
export function createSquareVisual(seed: SquareSeed): SquareVisual {
  return Signal<SquareState>({ ...seed, active: false, y: 0, scale: 1, strength: SQUARE_DIM })
}
...
/** Resolved tone values from the host's --boop-xterm-squares-*-tone tokens. */
export type SquarePalette = { user: string; agent: string; tool: string; other: string }

/** The wave 3 painter uses these values after resolving the host theme. */
export function themedSquareVars(state: SquareState, palette: SquarePalette): Record<string, string> {
  return {
    ...squareVars(state),
    "--asq-color": `hsl(${state.hue} ${palette[state.kind]})`,
    "--asq-shape": `var(--boop-xterm-squares-${state.kind}-shape)`,
  }
}
